...
class DriveEnv:
    ...
    def getDerivatives(self, t, y, dqn):
        linearVel = y[0]
        angularVel = y[1]
        theta = y[2]
        xPos = y[3]
        ...
        if self.controllerUpdateNum * self.dt <= t:
            ...
            action = dqn.choose_action(currentFrameState)
            ...
            if leftVoltage < 0 or rightVoltage < 0 or leftVoltage > 12 or rightVoltage > 12:
    ...
    def runEpisode(self, dqn):
        ...
        ivpOutput = integrate.solve_ivp(self.getDerivatives, timeSpan, self.startingState, method='RK45', args=(dqn,), max_step=self.dt / 10, atol=1e-8, t_eval=evalTimes)
        ...
        for i in range(1, len(self.controllerRecords) - 1):
            ...
            if 255 not in currFrameState:
                ...
                if currentDist > 2:
                    reward = -1
                print(f"CURRENT DIST: {currentDist} {curr[0]} {self.targetPose}")
                memory.append([currFrameState, action, reward, nextFrameState])
                break
            else:
                memory.append([currFrameState, action, reward, nextFrameState]) 
